			   	...
			   	<table id="page_manager1" style="width:239px !important; border:none; margin-top:-5px">     
                        @foreach($category as $categories)
                        	<tr id="{{$categories->fldCategoryID.'_'.$categories->fldCategoryPosition}}">
                            	<td style="padding: 5px 3px;"> <input type="checkbox" class="check-select" required="required"  name="category[]" value="{{ $categories->fldCategoryID }}" {!! DB::table('tblProductCategory')->where('fldProductCategoryCategoryID','=',$categories->fldCategoryID)->where('fldProductCategoryProductID','=',$product_id)->count()==1 ? "checked='checked'" : "" !!} /></td>
                        ...
                        <div style="margin-bottom:15px; margin-left:55px;padding: 5px 3px;">
                        	<button type="button" class="uk-button uk-button-success uk-button-small" name="button" onClick="addSubCategory({!! $category_id !!},{!! $product_id !!})">Add New Sub Category</button>
                         </div>
