    ...
    <div class="uk-grid">
      <div class="uk-width-1-1 uk-margin-medium-bottom">
        <div class="uk-grid">
           ...
           <label class="uk-display-inline uk-margin-medium-right light light-grey">Display By: </label>
           <div class="uk-button-dropdown" data-uk-dropdown>
            ...
            <div class="uk-button-dropdown  uk-margin-small-left" data-uk-dropdown>
              <button class="uk-button uk-button-trans lato"  type="button">Alphabetical <i class="ion ion-android-arrow-dropdown uk-margin-small-left"></i></button>
              <div class="uk-dropdown uk-dropdown-small">
                <ul id="products_sort" class="uk-nav uk-nav-dropdown bg-dark filter-btn">
                    <?php if (empty($slug) || $slug=='za') { ?>
                        <li class="uk-active"><a href="#" onclick="azOrder();">Product Name (A-Z)</a></li>
                        <li><a href="#" onclick="zaOrder();">Product Name (Z-A)</a></li>
                    <?php } else { ?>
          ...
          if (isset($product_price_from) && $product_price_from > 0) {
              $product_figure_html .= '<div class="price">From ';
                ...
                if($product_price_to > 0 && $product_price_to != $product_price_from){
                  if(is_numeric($product_price_from)){
                    $product_figure_html .= ' <span class="price-range-to">to</span> <span class="bold">$'.number_format($product_price_to,2).'</span>';
                  }else{ $product_figure_html .= ' <span class="price-range-to">to</span> <span class="bold">$'.$product_price_to.'</span>';
                  }
                }
...
<div class="gallery-style-1">
  <div class="uk-grid oading-products"  id="products">
       ...
       <div class="uk-width-1-1 uk-margin-large-bottom uk-margin-large-top">
             ...
             @if($product->total()  > 1)
                <li><a href="{{ $slug != '' ? url('/featured-images/'.$slug.'?page=1') : url('/featured-images?page=1') }}"> <i class="uk-icon uk-icon-chevron-left  pe-7s-angle-left"></i> </a></li>
             @endif
             ...
             @endfor
              ...
              @if($product->total()  > 1)
                <li><a href="{{ $slug != '' ? url('/featured-images/'.$slug.'?page='.$product->lastItem()) : url('/featured-images?page='.$product->lastPage()) }}"> <i class="uk-icon uk-icon-chevron-right  pe-7s-angle-right"></i> </a></li>
              @endif
             </ul>
