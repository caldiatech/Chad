	...
	<div class="uk-width-1-1">
		...
		<div class="uk-grid">
			<?php 
				...
				$vertical_width = 1;
				if($total_vertical_products_half > 0){						
					if($total_vertical_products_half >= 4){
						$vertical_width = 1;
					}elseif($total_vertical_products_half >= 3){
						$vertical_width = '1-3';
					}elseif($total_vertical_products_half >= 2){
						$vertical_width = '2';
					}elseif($total_vertical_products_half >= 1){
						$vertical_width = 4;
					}
				}
					...
					if(count($featured_photos_horizontal) > 0){
						if(isset($featured_photos_horizontal[$row_counter])){
							...
							<div class="uk-width-6-10 photo-{{$row_column_counter}} product-type-{{$this_product_vertical_item->fldProductIsVertical}} first-row">
								<div class="uk-overlay-hover  uk-cover-background  uk-scrollspy-inview uk-animation-fade" data-uk-modal="{target:'#modal-1'}" style="background-image:url('{{PRODUCT_IMAGE_PATH.$this_product_vertical_item->fldProductSlug.'/'.MEDIUM_IMAGE.$this_product_vertical_item->fldProductImage}}')">
