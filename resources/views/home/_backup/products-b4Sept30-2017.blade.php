        ...
        <div class="uk-grid">
          <div class="uk-width-medium-6-10 filter-section uk-width-1-1 uk-width-medium-1-1 uk-margin-medium-top">
		   ...
		   @endif
                   ...
                   <!--<li class="uk-active" data-uk-sort="my-category">-->
                   <li class="uk-active" data-uk-filter="{{$category_item->fldCategorySlug}}">
...
<div class="gallery-style-1">
  <div class="uk-grid uk-padding-remove "  id="products">
           ...
           @else
            ...
            <div class="uk-grid-width-small-1-2 uk-grid-width-medium-1-3 uk-grid-width-large-1-4 tm-grid-heights uk-masonry-gallery" data-uk-grid="{gutter: 20, controls: '.filter-btn'}">
               ...
               @foreach($product as $products)
                    <div class="product-list-item"  data-my-category="{{ $products->fldProductName }}" data-my-category2="{{ $products->fldProductPrice }}" data-uk-filter="{{$products->fldCategorySlug}}">
                        ...
                        <div class="uk-overlay-hover  uk-cover-background  uk-scrollspy-inview uk-animation-fade" >
                          <figure class="uk-overlay">
                              @if($products->fldProductImage != "" && (File::exists(PRODUCT_IMAGE_PATH.$products->fldProductSlug.'/'.MEDIUM_IMAGE.$products->fldProductImage)))
                                    {!! Html::image(PRODUCT_IMAGE_PATH.$products->fldProductSlug.'/'.MEDIUM_IMAGE.$products->fldProductImage,$products->fldProductName,array('id'=>'image'.$nctr,'class'=>'uk-align-center w100 hauto pull-left','width'=>'456')) !!}
                              @else
                    ...
                    <div class="uk-modal-header"><h2>'.strip_tags($products->fldProductName).'</h2></div>';
       ...
       <div class="uk-width-1-1 uk-margin-large-bottom uk-margin-large-top">
             ...
             @if($product->total()  > 1)
                <li><a href="{{ $slug != "" ? url('/images/'.$slug.'?page=1') : url('/images?page=1') }}"> <i class="uk-icon uk-icon-chevron-left  pe-7s-angle-left"></i> </a></li>
             @endif
             @for($i=1;$i<=$product->lastPage();$i++)
                @if($i==$product->currentPage())
                    <li class='uk-active'><span>{{ $i }}</span></li>
                @else
                  <li><a href="{{ $slug != "" ? url('/images/'.$slug.'?page='.$i) : url('/images?page='.$i) }}">{{ $i }}</a></li>
                @endif
             @endfor

              @if($product->total()  > 1)
                <li><a href="{{ $slug != "" ? url('/images/'.$slug.'?page='.$product->lastItem()) : url('/images?page='.$product->lastPage()) }}"> <i class="uk-icon uk-icon-chevron-right  pe-7s-angle-right"></i> </a></li>
              @endif
             </ul>
