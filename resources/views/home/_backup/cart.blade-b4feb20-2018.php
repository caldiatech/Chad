 ...
 <div class="uk-width-1-1 uk-margin-large  uk-margin-large-top">
    ...
    <div class="uk-width-1-1">
        ...
        @if (session()->has('message'))
            <div id="" style="padding:15px;" class="uk-alert uk-text-center uk-text-large uk-alert-success uk-margin-large-bottom"><i class="uk-icon uk-icon-check-circle"></i> <strong>Success:</strong>Your shopping card has been updated.</div><!-- text updated based on QAR -->
        @endif
        ...
        <table class="uk-table cart-order-list">
              ...
              @foreach($cart as $carts)
                ...
                <tr>
                    <td class="tp5 uk-vertical-align">
                         <a href="{{url('shopping-cart/delete/'.$carts->temp_cart_id)}}" class="uk-vertical-align-middle" onClick="return confirm(&quot;Are you sure you want to remove this products from your shopping cart?\n\nPress OK to delete.\nPress Cancel to go back without deleting the products from your shopping cart.\n&quot;)"><i class="uk-icon-close"></i></a>
                    <td class="tp1 uk-vertical-align">
                        <div class="uk-vertical-align-middle">
                            ...
                            @if($carts->fldTempCartFrameInfo != "")
                                 {!! Html::image('https://pod.cloud.graphikservices.com/renderEMF/render?imgUrl='.url(PRODUCT_IMAGE_PATH.$carts->fldProductSlug.'/'.SMALL_IMAGE.$carts->image).'&imgHI='.$carts->image_height.'&imgWI='.$carts->image_width.'&maxW=225&maxH=225&t='.$carts->fldTempCartMatBorderSize.'&r='.$carts->fldTempCartMatBorderSize.'&b='.$carts->fldTempCartMatBorderSize.'&l='.$carts->fldTempCartMatBorderSize.'&sku='.$carts->fldTempCartFrameInfo.'&frameW='.$carts->frame_size.$carts->matParams) !!}
                            @else
                                {!! Html::image(PRODUCT_IMAGE_PATH.$carts->fldProductSlug.'/'.SMALL_IMAGE.$carts->image) !!}
                            @endif
                            </div>
                            <div class="uk-width-large-6-10 uk-width-medium-1-2 uk-width-1-1 uk-float-left  cart-text-holder uk-padding-large-left  uk-padding-large-top uk-vertical-align">
                                ...
                                <div class="cart-item-more-info">{{$carts->fldTempCartFrameDesc}}</div>
                                <div class="w100 grey">{{ $carts->product_sub_title }}</div>
                            </div>
                        </div>
                    </td>
            ...
            <tfoot>
                <tr class="border-top border-bottom">
                    ...
                    <td  colspan="2" class="total-cart-price uk-text-right">   {!! Form::submit('UPDATE CART',array('class'=>'uk-button uk-button-grey uk-button-primary','name'=>'update'))!!}</td>
                </tr>
            </tfoot>
           @endif
        </table>
            ...
            <div class="uk-width-large-1-2 uk-width-1-1 uk-float-right">
                    ...
                    */ ?>
                     <div class="uk-grid uk-margin-remove  normalize sub-total-div border-bottom border-bottom-small" id="coupon_code">
                        <div class="uk-width-3-10  uk-padding-remove  uk-width-1-1">Discount</div>  <div class="uk-width-7-10 uk-width-1-1 bold total-cart-price payment"><span id="coupon_amount">$ 0.00</span></div>
                    </div>
