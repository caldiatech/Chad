...
@section('content')
   ...
   {!! Form::open(array('url' => '/image-galleries', 'method' => 'post', 'id' => 'pageform', 'class' => 'row-fluid bill-info')) !!}
        ...
        <div class="uk-grid">
          ...
          <div class="uk-width-medium-4-10  uk-width-1-1 bg-black input-100 uk-margin-medium-top">
            <div class="text-wrapper uk-width-small-1-2 uk-width-1-1 uk-float-right">
                ...
                {!! Form::text('search',"",array('id'=>'search','required','class'=>'uk-padding-medium-left','placeholder'=>'SEARCH')) !!}
...
<div class="gallery-style-1">
  <div class="uk-grid uk-padding-remove "  id="products">
       ...
       <div class="uk-width-1-1 uk-margin-large-bottom uk-margin-large-top">
             ...
             @if($product->total()  > 1)
                <li><a href="{{ $slug != "" ? url('/image-galleries/'.$slug.'?page=1') : url('/image-galleries?page=1') }}"> <i class="uk-icon uk-icon-chevron-left  pe-7s-angle-left"></i> </a></li>
             @endif
             ...
             @endfor
              ...
              @if($product->total()  > 1)
                <li><a href="{{ $slug != "" ? url('/image-galleries/'.$slug.'?page='.$product->lastItem()) : url('/image-galleries?page='.$product->lastPage()) }}"> <i class="uk-icon uk-icon-chevron-right  pe-7s-angle-right"></i> </a></li>
              @endif
             </ul>
