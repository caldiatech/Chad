...
var ffilter_array = new Array();
    ...
    function changeFrame(newSku,newFrameWidth,price,frameDesc) {
        ...
        enlargedImageURL = imageURL.replace(RegExp("imgHI=4&imgWI=12&maxW=400&maxH=400", "g"), replace_width);
...
window.sortMeBy = (function(){
  ...
  function generateNewImageFrame() {
    ...
    var is_prod_vertical_var = $('#is_prod_vertical').val();
    var replace_width = "imgHI="+imageHeight+"&imgWI="+imageWidth;
    if(is_prod_vertical_var == 1){
      replace_width = "imgHI="+imageWidth+"&imgWI="+imageHeight;
    }

    var newSrc = "https://pod.cloud.graphikservices.com/renderEMF/render?imgUrl={{ url(PRODUCT_IMAGE_PATH.$product->fldProductSlug.'/'.MEDIUM_IMAGE.$product->fldProductImage) }}&"+replace_width+"&maxW=400&maxH=400&m1b=1&off=0.375&sku="+$("#mainImage").attr('data-sku')+"&frameW="+$("#mainImage").attr('data-width');
        console.log(newSrc);
