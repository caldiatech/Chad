...
@section('content')
  ...
  <div class="uk-width-1-1 uk-margin-large  uk-margin-large-top"> 
                    ...
                    <div class="uk-grid">
                        ...
                        <div class="uk-width-medium-5-10 uk-width-1-1 uk-margin-top">    
                            <div class="box-bordered padding-medium ">        
                                <h4>New Password</h4>
                                <p>{!! (isset($pages->fldPagesDescription)) ? $pages->fldPagesDescription: '' !!}</p>
                                 @if(Session::has('error')) 
                                    <div class="uk-alert uk-alert-danger">{!!Session::get('error')!!}</div>
                                 @endif                      
                                 {!! Form::open(array('url' => '/shop-owner-new-password', 'method' => 'post',  'class' => 'row-fluid account-login input-100')) !!}               
                                <div class="formbox">
                                    <div class="uk-padding-small-top uk-grid uk-margin-remove">
                                        <div class="uk-width-large-1-4 uk-width-medium-1-1  uk-width-small-1-4  uk-width-1-1 uk-padding-remove ">
                                         {!! Form::label('password', '* Password',array('style'=>'width:75px')); !!}
                                        </div>
                                        <div class="uk-width-large-2-3 uk-width-medium-1-1  uk-width-small-2-3  uk-width-1-1 uk-padding-remove">
           ...
           <input type="hidden" name="shop_owner_id" value="{{ $shopOwner->fldShopOwnerID }}">
           <input type="hidden" name="hash" value="{{ $shopOwner->fldShopOwnerHashSecurity }}">  
