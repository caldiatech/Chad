            ...
            <a href="javascript:void(0)" class="uk-button">
             ...
             <span class="encircled">
               ...
               @if($client->fldClientImage  != "")
                {!! HTML::image(url(CUSTOMER_IMAGE_PATH.$client->fldClientID.'/'.THUMB_IMAGE.$client->fldClientImage),  isset($client) ? $client->fldClientFirstname . ' ' . $client->fldClientLastname : "" , array('class' => 'dashbaord-profile','width'=>'39', 'height'=>'39','style'=>'height:39px !important')) !!}
              
              @endif
             
            </span> 

              <span class="profile-name">{{ isset($client) ? $client->fldClientFirstname . ' ' . $client->fldClientLastname  : ""}}</span> <i class="uk-icon-angle-down"></i></a>
            <div class="uk-dropdown uk-dropdown-small uk-padding-remove">

                <ul class="uk-nav uk-nav-dropdown">
                    <li class="profile-hide "><a class="profile-name" href="{{url('dashboard/'.$pages->category.'/profile')}}" > <i class="uk-icon-user uk-icon-justify"></i> {{ isset($client) ? $client->fldClientFirstname . ' ' . $client->fldClientLastname : ""  }}</a></li>
                    <li><a href="{{url('dashboard/'.$pages->category.'/edit-profile')}}" class="@if($pages->slug == 'edit-profile') uk-active @endif"> <i class="uk-icon-pencil ion ion-edit uk-icon-justify"></i> Edit Profile</a></li>
                    <li><a href="{{url('dashboard/'.$pages->category.'/settings')}}" class="@if($pages->slug == 'settings') uk-active @endif"> <i class="uk-icon-wrench ion ion-settings uk-icon-justify"></i> Settings</a></li>
                    <li><a href="{{url('dashboard/'.$pages->category.'/logout')}}" class=""> <i class="uk-icon-sign-out uk-icon-justify"></i> Log Out</a></li>
                </ul>
            </div>
        ...
        <article id="main" role="main">
          <div class="uk-breadcrumb-wrapper">
            <ul class="uk-breadcrumb uk-margin-remove">
                <li class="uk-active">
		     @if($pages->category == "customer")
                    @if($pages->fldPagesTitle == "Dashboard")           
                      <span>&nbsp;</span>
                    @elseif($pages->fldPagesTitle == "Profile")             
                       <span><a href="{{ url('dashboard/customer') }}">Dashboard</a></span>
                       <span>/ Profile</span>
		     @elseif($pages->fldPagesTitle == "Update Profile")             
                       <span><a href="{{ url('dashboard/customer') }}">Dashboard</a></span>
                       <span>/ Update Profile</span>	
                    @elseif($pages->fldPagesTitle == "Order History")    
                       <span><a href="{{ url('dashboard/customer') }}">Dashboard</a></span>
                       <span>/ Order History</span>
                    @elseif($pages->fldPagesTitle == "Settings")    
                       <span><a href="{{ url('dashboard/customer') }}">Dashboard</a></span>
                       <span>/ Settings</span>   
                    @endif
                  @else
