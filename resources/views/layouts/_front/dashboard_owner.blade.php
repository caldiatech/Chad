        ...
        <div class="uk-button-dropdown" data-uk-dropdown>
            <a href="javascript:void(0)" class="uk-button"> 
              <span class="encircled">
                ...
                {!! HTML::image(url(SHOP_OWNER_IMAGE_PATH.$shopOwner->fldShopOwnerID.'/'.THUMB_IMAGE.$shopOwner->fldShopOwnerImage),  isset($shopOwner) ? $shopOwner->fldShopOwnerFirstname . ' ' . $shopOwner->fldShopOwnerLastname : "" , array('class' => 'dashbaord-profile','width'=>'39', 'height'=>'39','style'=>'height:39px !important')) !!}
                 @endif
              </span> 
             
              <span class="profile-name">{{ isset($shopOwner) ? $shopOwner->fldShopOwnerFirstname . ' ' . $shopOwner->fldShopOwnerLastname  : ""}}</span> <i class="uk-icon-angle-down"></i></a>
            <div class="uk-dropdown uk-dropdown-small uk-padding-remove">

                <ul class="uk-nav uk-nav-dropdown">
                    <li class="profile-hide "><a class="profile-name" href="{{url('dashboard/'.$pages->category.'/profile')}}" > <i class="uk-icon-user uk-icon-justify"></i> {{ isset($shopOwner) ? $shopOwner->fldShopOwnerFirstname . ' ' . $shopOwner->fldShopOwnerLastname : ""  }}</a></li>
                    <li><a href="{{url('dashboard/shop-owner/edit-profile')}}" class="@if($pages->slug == 'edit-profile') uk-active @endif"> <i class="uk-icon-pencil ion ion-edit uk-icon-justify"></i> Edit Profile</a></li>
                    <li><a href="{{url('dashboard/shop-owner/settings')}}" class="@if($pages->slug == 'settings') uk-active @endif"> <i class="uk-icon-wrench ion ion-settings uk-icon-justify"></i> Settings</a></li>
                    <li><a href="{{url('dashboard/shop-owner/logout')}}" class=""> <i class="uk-icon-sign-out uk-icon-justify"></i> Log Out</a></li>
                </ul>
            </div>
        ...
        <article id="main" role="main">
          <div class="uk-breadcrumb-wrapper">
            <ul class="uk-breadcrumb uk-margin-remove">
                <li class="uk-active">
		             @if($pages->category == "shop-owner")
                    @if($pages->fldPagesTitle == "Dashboard")           
                      <span>Dashboard</span>
                    @else
                       <span><a href="{{ url('dashboard/shop-owner') }}">Dashboard</a></span>
