...
class Cart extends Eloquent
{
	...
	static function getReturnFrameImage($orderCode,$slug,$image) {
		...
		$imageLink = 'https://pod.cloud.graphikservices.com/renderEMF/render?imgUrl='.url(PRODUCT_IMAGE_PATH.$slug.'/'.SMALL_IMAGE.$image).'&imgHI='.$imageSize->fldOptionsAssetsHeight.'&imgWI='.$imageSize->fldOptionsAssetsWidth.'&maxW=225&maxH=225&t='.$orderInfo->fldCartMatBorderSize.'&r='.$orderInfo->fldCartMatBorderSize.'&b='.$orderInfo->fldCartMatBorderSize.'&l='.$orderInfo->fldCartMatBorderSize.'&sku='.$orderInfo->fldCartFrameInfo.'&frameW='.$frameDESC[0].$matParams;		
		return $imageLink;		
