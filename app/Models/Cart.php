...
class Cart extends Eloquent
{
	...
	static function displayCart($order_code) {
		...
		if(count($cartDisplay) > 0){
			foreach($cartDisplay as $cartDisplays) {
				...
				dd($cartDisplays->quantity ,$cartDisplays->product_price);
	...
	static function getReturnFrameImage($orderCode,$slug,$image) {
		...
		$imageLink = 'https://pod.cloud.graphikservices.com/renderEMF/render?imgUrl=https://usd.mtg.mybluehost.me/Chad/public/'.PRODUCT_IMAGE_PATH.$slug.'/'.SMALL_IMAGE.$image.'&imgHI='.$imageSize->fldOptionsAssetsHeight.'&imgWI='.$imageSize->fldOptionsAssetsWidth.'&maxW=225&maxH=225'.$frameparams.$matParams;
