	...
	static function displayCart() {
		...
		$cartDisplay = self::leftJoin('tblProduct','tblTempCart.fldTempCartProductID','=','tblProduct.fldProductID')
							...
							->select('tblProduct.fldProductID as product_id','tblTempCart.fldTempCartID as temp_cart_id',
									 'tblTempCart.fldTempCartQuantity as quantity','tblTempCart.fldTempCartProductPrice as product_price','tblTempCart.graphik_cost',
									 ...
									 'tblTempCart.fldTempCartMatBorderSize', 'tblTempCart.fldTempCartFinishkitInfo', 'tblTempCart.fldTempCartLinerDesc', 'tblTempCart.fldTempCartLinerSku')->get();
		...
		if($cartDisplay->isEmpty())
			...
			foreach($cartDisplay as $cartDisplays) {
				...
				$discountAmount = $cartDisplays->product_price;
				$discountAmount = (float)str_replace(",", "", $discountAmount);
				
				$total =  ($cartDisplays->quantity * $discountAmount);
			...
			if(Session::has('couponCode')) {
				...
				Log::debug($cartDisplay[0]['coupon_amount']);
