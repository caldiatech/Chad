...
class TempCartController extends Controller
{
	...
	public function addShoppingCart(Request $request) {
		...
		$get_the_same_cart_item = TempCart::where('fldTempCartProductID','=', $product_id)->where('fldTempCartFrameInfo','=',$frame_info)->where('fldTempCartImageSize', '=', $frame_border_size)->first();
	...
	public function payment() {
        ...
        Log::debug('---------temp_cart--------------');
		// Authorize.net - START
		$subtotal_amount	= $tax_amount 		= $grandtotal = 0;
		if(isset($temp_cart[0]['grandtotal'])){
			$grandtotal	= $temp_cart[0]['grandtotal'];
		}
        ...
        Log::debug($grandtotal);
		if(isset($temp_cart[0]['tax'])){
			$tax_amount 		= $temp_cart[0]['tax'];
		}else{
        Log::debug('---------state--------------');
        Log::debug($state);
        ...
        Log::debug($tax_amount);
		$charge_amount 		= $grandtotal + $tax_amount + $shipping_amount;		
