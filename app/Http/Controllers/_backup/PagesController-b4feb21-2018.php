...
class PagesController extends Controller
{
   ...
   public function home() {
		...
		$product_vertical = Product::join('tblProductCategory','tblProductCategory.fldProductCategoryProductID','=','tblProduct.fldProductID')
						...
						->where('fldProductIsFeatured','=',1)->where('fldProductIsVertical','=',1)->take(4)->get();
		$product_horizontal = Product::join('tblProductCategory','tblProductCategory.fldProductCategoryProductID','=','tblProduct.fldProductID')
						->join('tblCategory','tblCategory.fldCategoryID','=','tblProductCategory.fldProductCategoryCategoryID')
						->where('fldProductIsFeatured','=',1)->where('fldProductIsVertical','!=',1)->take(1)->get();
		...
		if ($validator->fails()) {   	  
			return Redirect::to('/faq')->withInput()->withErrors($validator,'faq');		                
	  		...
	  		Mail::send('home.emails.faq', $contacts, function ($message) {									
		 		$message->from("no-reply@clarkin.com","Clarkin Web Admin");
		 		$message->to("test1@dogandrooster.net","Clarkin Admin");
		 		$message->cc("dennis@dogandrooster.com","Clarkin Admin 2");
