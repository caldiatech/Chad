...
class ManagerController extends Controller
{
    ...
    {   
		...
		$manager = Manager::where('fldManagerType','=',1)->get();
	...
	public function getSales($manager_id) {
		...
		//print_r($cart);die();
		$managerClass = 'class=active';
		return View::make('_admin.manager.sales', compact('manager','administrator','managerClass','cart'));	 
	}
   ...
   public function getEdit($id) {
	   ...
	   $managerClass = 'class=active';
	    ...
	    return View::make('_admin.manager.manager_edit', array('manager' => $manager,'administrator'=>$administrator,'managerClass'=>$managerClass,'braintreeClient'=>$braintreeClient,'braintreeMerchant'=>$braintreeMerchant));		
   }
   ...
   public function postEdit($id) {	  
		...
		return Redirect::to('dnradmin/manager/edit/'.$id)->withInput()->withErrors($validator,'manager');	
		   ...
		   if(Input::get('status') == 2) {
		   	  ...
		   	  Mail::send('home.emails.manager.activate', $messageData, function ($message) use($settings,$manager) {
							...
							$message->to("boyet@dogandrooster.com",$manager->fldManagerFirstname . ' ' . $manager->fldManagerLastname)->subject("Welcome to Clarkin");
				});
				...
				$settings = Settings::first();									
			 	 ...
			 	 //send email code goes here			  							
				  		Mail::send('home.email_manager_registration', $messageData, function ($message) use($settings) {
							...
							$message->from(EmailFrom, EmailFromName);
							$message->to(Input::get('email'),Input::get('firstname') . ' ' . Input::get('lastname'))->subject("Welcome to Clarkin");									
						});
						...
						Mail::send('home.email_manager_registration_owner', $messageData, function ($message) use($settings) {
							...
							$message->to($ownerEmail,$ownerName)->subject("New Sales Manager");									
						});
		   Session::flash('success',"Your information is successfully saved. Our representative will contact you as soon as possible."); 		   
	...
	public function salesLogin() {
		...
		if ($validator->fails()) {    		
			...
			//echo $manager->fldManagerPassword;die();
	...
	public function updateAccounts() {
		...
		if ($validator->fails()) {    		
			...
			Session::flash('success',"Accounts was successfully saved.");  
			return Redirect::to('/dashboard/sales/accounts'); 
		}
	 }
