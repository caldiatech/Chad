...
class ManagerController extends Controller
{
    ...
    {   
		...
		foreach($manager as $managers) {
			//check commission
			$commission = ManagerCommission::calculateCommissionYear($managers->fldManagerID);
   ...
   public function postEdit($id) {	  
				...
				$settings = Settings::first();									
			 	 ...
			 	 //send email code goes here			  							
				  		Mail::send('home.email_manager_registration', $messageData, function ($message) use($settings) {
							...
							$message->to(EmailTo,EmailToName)->subject("New Sales Manager");									
						});
	...
	public static function orderDetails($orderNo) {
	 	...
	 	$orderInfo->orderDetails = Cart::getImageSize($orderInfo->fldCartImageSize) . ', '. 
