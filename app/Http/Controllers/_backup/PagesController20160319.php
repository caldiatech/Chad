...
class PagesController extends Controller
{
   ...
   public function postNew() {
	   ...
	   list($width, $height) = getimagesize($path);
		...
		$height = "250";
		   ...
		   if($width <= "3000" && $height <= "3000") {	   	
			   ...
			   if($file != "") {
				...
				if ($validation->fails()){					
					return Redirect::to('dnradmin/pages/new')->withErrors($validation);
				} else {	
					//get last position
		...
		Session::flash('error',"Alert: your image too  large!");   	   				
		   return Redirect::to('dnradmin/pages/new')->withInput();	
	   }
   }
   ...
   public function postEdit($id) {	  
	  ...
	  if($file != "") {
		  ...
		  list($width, $height) = getimagesize($path);
		   if($width > "3000" && $height > "3000") {
			   Session::flash('error',"Alert: your image too  large!");  	   				
		   	   return Redirect::to('dnradmin/pages/edit/'.$id)->withInput();	
   ...
   public function forgot_password() {
	   $menus = Pages::where('fldPagesMainID', '=', 0)->get();
		...
		$cart_count = TempCart::countCart();				
   		return View::make('home.forgot')->with(array('menus'=>$menus,'category'=>$category,'settings'=>$settings,'google'=>$google,'cart_count'=>$cart_count)); 
   }

   /**************CODE FOR INLINE EDITING************************/
   public function savePageChanges() {
	   		...
	   		$pages= Pages::find($id);
	   			...
	   			if($title != "") {
	   				$pageCount = Pages::where('fldPagesName','=',$title)->where('fldPagesID','!=',$id)->count();
	   				$slug = $pageCount == 0 ? str_slug($title,'-') : str_slug($title."-".$pageCount,'-');

	   				$pages->fldPagesTitle = $title;
	   				$pages->fldPagesSlug = $slug;
	   				$pages->fldPagesDescription = $description;
	   			} else {
	   				$pages->fldPagesDescription = Input::get('description_home');	   				
	   			}
   ...
   public function savePageChangesWithoutTitle() {
   		$id = Input::get('Id');
   ...
   public function uploadBanner($id) {
   		...
   		if($file != "") {
		   $path = Input::file('banner')->getRealPath();
		   list($width, $height) = getimagesize($path);
		} 
