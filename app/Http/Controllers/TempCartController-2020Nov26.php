...
class TempCartController extends BaseController
{
	...
	private function __paymentSendMail($order_code) {
		...
		Mail::send('home.email_checkout', $dataFields, function ($message) use($settings) {
			...
			if ($settings->shop_email) { $message->to($settings->shop_email ,$settings->shop); }
			if ($settings->manager_email) { $message->to($settings->manager_email ,$settings->manager); }
		...
		// 	$message->bcc('dennis+9999@dogandrooster.com', 'DNR Dev');
		// 	$message->bcc('team.a+9999@dogandrooster.com', 'DNR Dev');
