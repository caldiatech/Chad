...
$a->add_field('x_login', '9hhD3AD4E422');
$a->add_field('x_tran_key', '78qYK4876MSSbm82');
...
$address = urlencode('8887 Royal Oak Way NW'); //ok
$city = urlencode('Calgary'); //ok
$state =urlencode('Alberta'); //ok
$zip = urlencode('T3G 0A4'); //ok
$country = urlencode('CA'); //ok
...
$a->add_field('x_email', 'test1@dogandrooster.net');
...
$a->add_field('x_exp_date', '1013');    // march of 2008
...
echo "<p><strong>outputs all the fields that we set</strong></p>";
$a->dump_fields();      // outputs all the fields that we set
echo "<br><br><strong>outputs the response from the payment gateway</strong>";
